...
export class ExchangeReader {
  ...
  async validateForAdmission(order: SignedOrder, hash: Hex): Promise<void> {
    ...
    if (order.expiration <= block.timestamp) {
      throw new ChainValidationError("order_expired", "order expiration is not in the future");
    }
  ...
  private async validateCeoOrder(order: SignedOrder, now: bigint): Promise<void> {
    ...
    try {
      ...
      if (order.side === Side.BUY) {
        ...
        await this.requireErc20Funds(collateral, order.maker, order.makerAmount);
      } else {
  ...
  private async validateErc20Order(order: SignedOrder): Promise<void> {
    ...
    try {
      ...
      await this.requireErc20Funds(asset, order.maker, order.makerAmount);
    } catch (error) {
      if (error instanceof ChainValidationError) throw error;
      throw new ChainValidationError(
        "chain_read_failed",
  ...
  private async requireErc20Funds(token: Address, maker: Address, required: bigint): Promise<void> {
