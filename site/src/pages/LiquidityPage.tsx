...
export function LiquidityPage() {
  ...
  return <main className="page-shell liquidity-page"><header className="filing-header"><p className="eyebrow">FORM FTC-DKT · ROBIN’S DOCKET AUCTION</p><h1>Propose liquidity</h1><p className="lede">Pay to deepen one company’s three market curves. Most of the payment increases `b`; the rest enters a competitive reward pool for useful surprise.</p></header>{error ? <section className="notice">The company register could not be read.</section> : null}<div className="liquidity-layout"><aside><label htmlFor="company-boost">Company filing</label><select id="company-boost" value={selected?.id ?? ''} onChange={(event) => setSelectedId(Number(event.target.value))}>{rows?.map((row) => <option key={row.id} value={row.id}>{row.ticker} · {row.name}</option>)}</select><p>Increasing `b` makes the curve deeper: a larger informed trade is required to move the displayed estimate by the same distance.</p></aside>{selected ? <BoostForm row={selected} /> : <p>Reading the docket…</p>}</div></main>
}
